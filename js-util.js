(function (exports) {
	const util = {
		...
		dateStringReplacements: {
			...
			'%c': function (date) {
				return util.dateStringReplacements['%x'](date) + ' ' + util.dateStringReplacements['%X'](date);
			},
			'%d': function (date) {
			...
			'%x': function (date) {
				return util.dateStringReplacements['%m'](date) + '/' + util.dateStringReplacements['%d'](date) + '/' + util.dateStringReplacements['%y'](date);
			},
			'%X': function (date) {
				return util.dateStringReplacements['%H'](date) + ':' + util.dateStringReplacements['%M'](date) + ':' + util.dateStringReplacements['%S'](date);
			},
			'%y': function (date) {
		...
		decToHex: function decToHex(dec, length) {
			length = length || 8;
			let output = (parseInt(dec) + Math.pow(16, length)).toString(16).substr(-length).toString();
		...
		randColor: function (opts) {
			...
			const h = util.randInt(Math.max(0, h_lvl - h_var), Math.min(360, h_lvl + h_var));

			const s_lvl = opts.s_lvl || 55;
			const s_var = opts.s_var || 25;
			const s = util.randInt(Math.max(0, s_lvl - s_var), Math.min(100, s_lvl + s_var));

			const l_lvl = opts.l_lvl || 55;
			const l_var = opts.l_var || 10;
			const l = util.randInt(Math.max(0, l_lvl - l_var), Math.min(100, l_lvl + l_var));
		...
		toCamelCase: function (string, upperCamelCase = false, splitter = ' ') {
			return string
				.split(splitter)
				.map((item, index) => {
					return (index === 0 && upperCamelCase) || index > 0 ? util.capitalize(item) : item;
				})
				.join('');
		},
		...
		toFixed: function (num, decimalPlaces, outputAsNumber) {
			// no rounding, yay!
			const floatRegex = new RegExp('(^-?[0-9]*)\\.?([0-9]{0,' + (decimalPlaces - 1 || -1) + '}[1-9]{1}(?=.+?0*$))?');
	...
	exports.jsUtil = util;
})(typeof exports === 'undefined' ? (this.jsUtil = {}) : exports);
